...
contract Twitter is Ownable {
    ...
    // Constructor for initializing the contract with the initial owner and profile contract
    constructor(address _profileContract) Ownable(msg.sender) {
        profileContract = IProfile(_profileContract);
    }

    // Change the maximum tweet length (onlyOwner can call)
    function changeTweetLength(uint16 newTweetLength) public onlyOwner {
        MAX_TWEET_LENGTH = newTweetLength;
    }

    // Get total likes for a user's tweets
    ...
    // Create a tweet (onlyRegistered users can create tweets)
    ...
    // Like a tweet (onlyRegistered users can like tweets)
    ...
    // Unlike a tweet (onlyRegistered users can unlike tweets)
    ...
    // Get a specific tweet by index (from the sender's tweets)
